<?php if ($type != 'hidden') { ?>
    ...
    ?>
    <input <?= !empty($options['required']) ? 'required' : ''; ?> type ="<?= $type; ?>" placeholder="<?= !empty($options['placeholder']) ? $options['placeholder'] : ''; ?>" class="form-control" name = '<?= $name; ?>' value = '<?= !empty($options['value']) ? addcslashes($options['value'], "'") : ''; ?>' />
    <?php
